...
import fluence.statemachine.AbciService.QueryResponse
import fluence.statemachine.Tx
import fluence.statemachine.Tx.Head
...
object AbciState {
  ...
  private implicit class LensSessions[F[_]: Functor, T](sessionsState: StateT[F, Sessions, T]) {
    ...
    def toAbciState: StateT[F, AbciState[F], T] =
   ...
   */
  def addTx[F[_]: Monad](tx: Tx, maxSessions: Int = 128, maxPendingTxs: Int = 24): StateT[F, AbciState[F], Boolean] =
    for {
      // Add tx to sessions
   ...
   */
  def boundResponses[F[_]: Monad](limit: Int): StateT[F, AbciState[F], List[(Tx.Head, Array[Byte])]] =
    StateT.get[F, AbciState[F]].flatMap {
  ...
  def putResponseSubscription[F[_]: Monad](head: Head,
                                           deferred: Deferred[F, QueryResponse]): StateT[F, AbciState[F], Unit] =
    for {
      _ ← StateT.modify[F, AbciState[F]](
        s ⇒ s.copy(requestSubscribtions = s.requestSubscribtions + (head -> deferred))
      )
    } yield ()
  ...
  def putResponse[F[_]: Monad](head: Tx.Head,
                               data: Array[Byte],
                               resultsLimit: Int = 512): StateT[F, AbciState[F], Unit] =
    for {
      _ ← StateT.modify[F, AbciState[F]](s ⇒ s.copy(responses = s.responses.enqueue(head -> data)))
   ...
   * @param hash App hash
   */
  def setAppHash[F[_]: Applicative](hash: ByteVector): StateT[F, AbciState[F], Unit] =
   ...
   */
  def formBlock[F[_]: Monad]: StateT[F, AbciState[F], List[Tx]] =
    for {
      state ← StateT.get[F, AbciState[F]]
      txs ← Sessions.commit[F](state.blockSessions.toList).toAbciState
      _ ← StateT.modify[F, AbciState[F]](s ⇒ s.copy(blockSessions = Set.empty))
